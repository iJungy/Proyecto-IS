...
//--------------Funciones para agrupar peliculas
// porgenero----------------------------
 ...
 * La idea es evitar volver a calcular la misma función con los mismos argumentos,
 * lo que puede ser costoso o redundante.
 * En su lugar, puede buscar el resultado almacenado en caché y devolverlo inmediatamente.
 */
int mochila(Movie movies[], int nmovies, int t, int index, int *select) {
  // con el calculo obteneido, solo regresamos
  if (range[index][t] != -1) {
    return range[index][t];
  }
  // el caso base que tenemos es que si completamos el tiempo exacto
  if (t == 0) {
    return range[index][t] = 1;
  }

  // si completamos todas las peliculas, hacemos que retorne 0
  if (index == nmovies || t < 0) {
    return range[index][t] = 0;
  }
  ...
  select[index] = 0;
  return range[index][t] = mochila(movies, nmovies, t, index + 1, select);
}

void calculartimet(Movie movies[], int nmovies, int t) {
  int select[nmovies];
  memset(select, 0, sizeof(select));
  // -1 quiere decir que no se ha calculado el estado en el que se encuentra
  memset(range, -1, sizeof(range));

  if (mochila(movies, nmovies, t, 0, select)) {
